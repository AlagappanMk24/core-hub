...
// sidebar.component.ts - Complete updated version
import { Component, OnInit, Input, Output, EventEmitter } from '@angular/core';
import { CommonModule } from '@angular/common';
import { Router, NavigationEnd } from '@angular/router';
import { filter } from 'rxjs/operators';
...
export class SidebarComponent implements OnInit {
  ...
  filterMenuItems(): void {
    ...
    if (userRoles.includes('User') && !userRoles.includes('Admin')) {
      ...
      if (customersItem) {
        customersItem.route = '/customers/view';
      }
    }
  }
  ...
  onMenuItemClick(item: MenuItem): void {
    if (item.hasSubmenu) {
      ...
      // If this is a subitem, mark parent as active
      this.menuItems.forEach((menuItem) => {
  ...
  private updateActiveMenuItem(currentUrl: string): void {
    this.menuItems.forEach((item) => {
      if (item.hasSubmenu && item.subItems) {
        let anySubItemActive = false;
        
        item.subItems.forEach((subItem) => {
          // Exact match for subitems to prevent /invoices matching /invoices/recurring
        ...
        // Parent is active only if a subitem is active
        item.active = anySubItemActive;
