...
import { InvoiceImportDialogComponent } from '../invoice-import/invoice-import.component';
...
export class InvoiceComponent implements OnInit {
  ...
  /**
   * Calculate total receivable amount (Pending + Partially Paid + Overdue)
   */
  getTotalReceivable(): number {
    const pendingAmount = this.stats.pending?.amount || 0;
    const partiallyPaidAmount = this.stats.partiallyPaid?.amount || 0;
    const overdueAmount = this.stats.overdue?.amount || 0;
    return pendingAmount + partiallyPaidAmount + overdueAmount;
  }

  /**
   * Calculate collection rate percentage
   * (Paid amount / Total amount excluding Draft) * 100
   */
  getCollectionRate(): number {
    const paidAmount = this.stats.paid?.amount || 0;

    // Calculate total amount excluding draft (only sent/invoiced amounts)
    const sentAmount = this.stats.sent?.amount || 0;
    const viewedAmount = this.stats.viewed?.amount || 0;
    const partiallyPaidAmount = this.stats.partiallyPaid?.amount || 0;
    const overdueAmount = this.stats.overdue?.amount || 0;
    const pendingAmount = this.stats.pending?.amount || 0;
    ...
    return Math.round((paidAmount / totalActiveAmount) * 100);
  }

  // Add these methods to your InvoiceComponent class

/**
 * Calculate total invoiced amount for customer (all invoices)
 */
getCustomerTotalInvoiced(): number {
  return this.stats.all?.amount || 0;
}

/**
 * Calculate total due amount for customer (pending + partially paid + overdue)
 */
getCustomerTotalDue(): number {
  const pendingAmount = this.stats.pending?.amount || 0;
  const partiallyPaidAmount = this.stats.partiallyPaid?.amount || 0;
  const overdueAmount = this.stats.overdue?.amount || 0;
  return pendingAmount + partiallyPaidAmount + overdueAmount;
...
/**
 * Calculate payment progress percentage for customer
 * (Paid amount / Total invoiced amount) * 100
 */
getCustomerPaymentProgress(): number {
  const totalInvoiced = this.stats.all?.amount || 0;
  const paidAmount = this.stats.paid?.amount || 0;
  
  if (totalInvoiced === 0) return 0;
  return Math.round((paidAmount / totalInvoiced) * 100);
}

/**
 * Calculate payment health score for customer
 * (On-time payments / Total paid invoices) * 100
 * On-time = paid before or on due date
 */
getCustomerPaymentHealth(): number {
  const paidInvoices = this.invoices.filter(
    inv => inv.paymentStatus === 'Paid' && inv.paidDate
  );
