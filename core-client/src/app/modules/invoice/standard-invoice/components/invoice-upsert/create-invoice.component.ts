...
import {
  ...
  DiscountTypeDisplay,
  InvoiceFormData,
} from '../../../../../shared/types/invoice-form.types';
...
// Models & Constants
import { CustomerFilterRequest } from '../../../../customer/services/models/customer.model';
import {
  AVATAR_COLORS,
...
export class CreateInvoiceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (customerId) {
        this.preloadCustomer(Number(customerId));
      }
    });
  }
  ...
  /**
   * Preload a customer by ID when creating invoice from customer details page
   */
  private preloadCustomer(customerId: number): void {
    this.customerService.getCustomerById(customerId).subscribe({
      next: (customer) => {
        const customerDisplay: CustomerDisplay = {
          id: customer.id,
          name: customer.name,
          address: `${customer.addressLine1}, ${customer.city}, ${customer.countryName}`,
          initials: this.getInitials(customer.name),
          color: this.getRandomColor(),
        };
        this.selectCustomer(customerDisplay);
        this.customerSearch = customer.name;
      },
      error: (err) => {
        console.error('Error preloading customer:', err);
      },
    });
  }

  // Helper method to get the initials from a name
  ...
  private mapCustomersToDisplay(customers: any[]): CustomerDisplay[] {
    return customers.map((c) => ({
      id: c.id,
      name: c.name,
      address: `${c.addressLine1 || ''}, ${c.city || c.address?.city || ''}, ${c.countryName || c.address?.country || ''}`,
      initials: c.name
        .split(' ')
        .map((n: string) => n[0])
        .join('')
  ...
  openCreateCustomerDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const address1 = result.address?.address1 || '';
        const address2 = result.address?.address2 || '';
        const city = result.address?.city || '';
        const country = result.address?.country || '';
        const fullAddress = [address1, address2, city, country]
          .filter((part) => part)
          .join(', ');
  ...
  toggleCustomerDropdown(): void {
    if (!this.showCustomerDropdown) {
      // ✅ When opening dropdown, show all customers if search is empty
      if (!this.customerSearch.trim()) {
        this.filteredCustomers = [...this.customers];
      } else {
        // Filter customers based on current search
        this.filteredCustomers = this.customers.filter(
          (c) =>
            c.name.toLowerCase().includes(this.customerSearch.toLowerCase()) ||
            c.address.toLowerCase().includes(this.customerSearch.toLowerCase()),
        );
      }
    }
  ...
  onCustomerSearchChange(): void {
    // ✅ When user types in search, filter the dropdown
    if (this.customerSearch && this.customerSearch.trim()) {
      const searchLower = this.customerSearch.toLowerCase().trim();
      this.filteredCustomers = this.customers.filter(
        (c) =>
      ...
      // ✅ When search is empty, show all customers
      this.filteredCustomers = [...this.customers];
    }
  ...
  selectCustomer(customer: CustomerDisplay): void {
    ...
    // 2. Update search input to show selected customer name
    this.customerSearch = customer.name;

    // 3. ✅ Clear the filtered customers list after selection
    this.filteredCustomers = [];

    // 4. Hide the validation message immediately
    ...
    this.updateUrlWithCustomerId(customer.id);
  }

  /**
   * Update URL with the selected customer ID
   */
  private updateUrlWithCustomerId(customerId: number): void {
    const urlTree = this.router.parseUrl(this.router.url);
    urlTree.queryParams['customerId'] = customerId.toString();
