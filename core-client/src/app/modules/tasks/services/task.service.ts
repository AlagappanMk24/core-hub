...
export class TaskService {
  ...
  // Helper method to convert Date to ISO string safely
 ...
 getTasks(filter: TaskFilterDto): Observable<PaginatedResult<Task>> {
  ...
  createTask(task: CreateTaskDto): Observable<Task> {
    ...
    const taskToSend = {
      ...task,
      dueDate: this.toISOString(task.dueDate),
      reminderDate: this.toISOString(task.reminderDate),
    };
  ...
  updateTask(id: number, task: UpdateTaskDto): Observable<Task> {
    ...
    return this.http.put<Task>(`${this.apiUrl}/${id}`, taskToSend);
  }
