// src/app/features/tasks/task-drawer/task-drawer.component.ts
...
import { UserListDto, UserService } from '../../../services/user/user.service';
...
isEditMode = false;
  ...
  priorityOptions = [
    { value: TaskPriority.Low, label: 'Low', icon: 'arrow_downward', color: '#10b981', bg: '#d1fae5' },
    { value: TaskPriority.Medium, label: 'Medium', icon: 'remove', color: '#3b82f6', bg: '#dbeafe' },
    { value: TaskPriority.High, label: 'High', icon: 'arrow_upward', color: '#f97316', bg: '#fed7aa' },
    { value: TaskPriority.Urgent, label: 'Urgent', icon: 'warning', color: '#ef4444', bg: '#fee2e2' }
  ];
 ...
 loadUsers(): void {
    ...
    const companyId = userDetail?.companyId ? parseInt(userDetail.companyId) : null;
 ...
 loadParentTasks(): void {
    ...
    this.taskService.getTasks(filter).subscribe({
      next: (tasks) => {
        if (this.isEditMode && this.taskId) {
          ...
          this.parentTasks = tasks.filter(t => t.id !== this.taskId && t.parentTaskId !== this.taskId);
        } else {
  ...
  saveTask(): void {
    ...
    if (this.isEditMode && this.taskId) {
      const updateDto: UpdateTaskDto = { ...this.task };
