...
namespace Core_API.Infrastructure.Services
{
    ...
    public class RecurringInvoiceService : IRecurringInvoiceService
    {
        ...
        public async Task<OperationResult<RecurringInvoiceResponseDto>> CreateAsync(RecurringInvoiceCreateDto dto, OperationContext context)
        {
            ...
            try
            {
                ...
                // Map DTO to entity
                var recurringInvoice = _mapper.Map<RecurringInvoice>(dto);
                ...
                await _hubContext.Clients.Group($"company-{companyId}").SendAsync(
                    ...
                    new { Id = recurringInvoice.Id, Name = recurringInvoice.Name });
        ...
        public async Task<OperationResult<RecurringInvoiceResponseDto>> UpdateAsync(RecurringInvoiceUpdateDto dto, OperationContext context)
        {
            using var transaction = await _unitOfWork.BeginTransactionAsync();
            try
            {
        ...
        public async Task<OperationResult<bool>> ResumeAsync(int id, OperationContext context)
        {
            try
            {
                ...
                await _hubContext.Clients.Group($"company-{companyId}").SendAsync(
                    ...
                    new { Id = id, NextInvoiceDate = recurringInvoice.NextInvoiceDate });
            ...
            try
            {
                    ...
                    recurringInvoice.OccurrencesGenerated >= recurringInvoice.MaxOccurrences.Value)
                {
                    ...
                    var completionLog = new RecurringInvoiceAuditLog
                    {
                        ...
                        Changes = JsonSerializer.Serialize(new { OccurrencesGenerated = recurringInvoice.OccurrencesGenerated }),
                        IpAddress = "system",
                        UserAgent = "system",
                        CreatedBy = context.UserId,
                        CreatedDate = DateTime.UtcNow
        ...
        public async Task<List<RecurringInvoice>> GetDueRecurringInvoicesAsync(DateTime asOfDate)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return new List<RecurringInvoice>();
            }
        }
        ...
        private bool ValidateSchedule(RecurringInvoiceCreateDto dto, out string errorMessage)
        {
            errorMessage = null;
        ...
        private DateTime CalculateNextDate(RecurringInvoice invoice, DateTime fromDate)
        {
        ...
        private bool HasScheduleChanged(RecurringInvoiceUpdateDto dto, RecurringInvoice existing)
        {
        ...
        private async Task<InvoiceCreateDto> CreateInvoiceFromTemplate(
        ...
        private async Task MarkGenerationFailed(int recurringInvoiceId, string errorMessage)
        {
            try
            {
        ...
        private async Task<RecurringInvoiceResponseDto> MapToResponseDto(RecurringInvoice invoice)
        {
