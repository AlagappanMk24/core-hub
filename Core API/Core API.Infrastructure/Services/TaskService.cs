// Core_API.Infrastructure/Services/Task/TaskService.cs
...
{
    public class TaskService(IUnitOfWork unitOfWork, ILogger<TaskService> logger) : ITaskService
    {
        ...
        public async Task<OperationResult<TaskDto>> GetTaskByIdAsync(int id, OperationContext context)
        {
            try
            {
                ...
                return OperationResult<TaskDto>.SuccessResult(MapToDto(task));
            }
            catch (Exception ex)
            {
        ...
        public async Task<OperationResult<TaskDto>> CreateTaskAsync(CreateTaskDto createDto, OperationContext context)
        {
            try
            {
                ...
                var task = new TaskItem
                {
                    ...
                    Tag = createDto.Tag,
                    AssignedToUserId = createDto.AssignedToUserId ?? context.UserId,
                ...
                return OperationResult<TaskDto>.SuccessResult(MapToDto(createdTask));
            }
            catch (Exception ex)
            {
        ...
        public async Task<OperationResult<TaskDto>> UpdateTaskAsync(int id, UpdateTaskDto updateDto, OperationContext context)
        {
            try
            {
                ...
                {
                    changes["Title"] = new { Old = task.Title, New = updateDto.Title };
                    task.Title = updateDto.Title;
                }

                if (updateDto.Description != null && task.Description != updateDto.Description)
                {
                    changes["Description"] = new { Old = task.Description, New = updateDto.Description };
                    task.Description = updateDto.Description;
                }

                if (updateDto.Priority.HasValue && task.Priority != updateDto.Priority.Value)
                {
                    changes["Priority"] = new { Old = task.Priority, New = updateDto.Priority.Value };
                    task.Priority = updateDto.Priority.Value;
                }

                if (updateDto.Status.HasValue && task.Status != updateDto.Status.Value)
                {
                    changes["Status"] = new { Old = task.Status, New = updateDto.Status.Value };
                ...
                {
                    changes["DueDate"] = new { Old = task.DueDate, New = updateDto.DueDate.Value };
                    task.DueDate = updateDto.DueDate.Value;
                }

                if (updateDto.Category != null && task.Category != updateDto.Category)
                {
                    changes["Category"] = new { Old = task.Category, New = updateDto.Category };
                    task.Category = updateDto.Category;
                }

                if (updateDto.Tag != null && task.Tag != updateDto.Tag)
                {
                    changes["Tag"] = new { Old = task.Tag, New = updateDto.Tag };
                ...
                {
                    changes["AssignedToUserId"] = new { Old = task.AssignedToUserId, New = updateDto.AssignedToUserId };
                ...
                {
                    changes["EstimatedHours"] = new { Old = task.EstimatedHours, New = updateDto.EstimatedHours.Value };
                ...
                {
                    changes["ActualHours"] = new { Old = task.ActualHours, New = updateDto.ActualHours.Value };
                ...
                if (changes.Any())
                {
                    await LogTaskAction(task.Id, "Updated", $"Task updated. Changes: {JsonSerializer.Serialize(changes)}", context);
                }

                var updatedTask = await _unitOfWork.TaskItems.GetTaskWithDetailsAsync(task.Id);
                return OperationResult<TaskDto>.SuccessResult(MapToDto(updatedTask));
            }
            catch (Exception ex)
            {
        ...
        {
            return await UpdateTaskAsync(id, new UpdateTaskDto { Status = status }, context);
        }
        public async Task<OperationResult<TaskDto>> AssignTaskAsync(int id, string userId, OperationContext context)
        {
            return await UpdateTaskAsync(id, new UpdateTaskDto { AssignedToUserId = context.UserId }, context);
        }
        ...
        public async Task<OperationResult<TaskCommentDto>> AddCommentAsync(int taskId, CreateTaskCommentDto commentDto, OperationContext context)
        {
            try
            {
                ...
                return OperationResult<TaskCommentDto>.SuccessResult(new TaskCommentDto
                {
                    Id = comment.Id,
        ...
        public async Task<OperationResult<TaskAttachmentDto>> AddAttachmentAsync(int taskId, IFormFile file, OperationContext context)
        {
            try
            {
                ...
                return OperationResult<TaskAttachmentDto>.SuccessResult(MapToAttachmentDto(attachment));
            }
            catch (Exception ex)
            {
        ...
        public async Task<OperationResult<IEnumerable<TaskAttachmentDto>>> GetAttachmentsAsync(int taskId, OperationContext context)
        {
            try
            {
                ...
                var attachmentDtos = attachments.Select(MapToAttachmentDto);
        ...
        public async Task<OperationResult<TaskStatsDto>> GetTaskStatsAsync(OperationContext context)
        {
            try
            {
                ...
                var stats = new TaskStatsDto
                {
                    ...
                    TasksAssignedToMe = tasks.Count(t => t.AssignedToUserId.ToString() == context.UserId),
                    TasksCreatedByMe = tasks.Count(t => t.CreatedByUserId.ToString() == context.UserId),
                    MyTasks = tasks.Count(t => t.AssignedToUserId.ToString() == context.UserId || t.CreatedByUserId.ToString() == context.UserId),
                    ...
                    OverdueTasks = tasks.Count(t => t.Status != TaskStatus.Completed &&
                                                    t.DueDate.HasValue &&
                    ...
                    TasksDueThisWeek = tasks.Count(t => t.DueDate.HasValue &&
                                                        t.DueDate.Value.Date >= today &&
                                                        t.DueDate.Value.Date <= endOfWeek),
                    TasksDueThisMonth = tasks.Count(t => t.DueDate.HasValue &&
                                                         t.DueDate.Value.Date >= today &&
                                                         t.DueDate.Value.Date <= endOfMonth),
        ...
        public async Task<OperationResult<IEnumerable<TaskDto>>> GetOverdueTasksAsync(OperationContext context)
        {
            try
            {
                ...
                return OperationResult<IEnumerable<TaskDto>>.SuccessResult(filteredTasks.Select(MapToDto));
            }
            catch (Exception ex)
            {
        ...
        public async Task<OperationResult<IEnumerable<TaskDto>>> GetMyTasksAsync(OperationContext context, TaskFilterDto? filter = null)
        {
            try
            {
                ...
                return OperationResult<IEnumerable<TaskDto>>.SuccessResult(tasks.Select(MapToDto));
            }
            catch (Exception ex)
            {
        ...
        private IQueryable<TaskItem> ApplyFilters(IQueryable<TaskItem> query, TaskFilterDto? filter)
        {
            ...
            //if (filter.MyTasks == true && int.TryParse(_unitOfWork.GetCurrentUserId(), out var userId))
            //{
        ...
        private TaskDto MapToDto(TaskItem task)
        {
            return new TaskDto
            {
                ...
                CreatedAt = (DateTime)task.CreatedDate,
                ...
                Attachments = task.Attachments?.Select(a => new TaskAttachmentDto
                {
                    ...
                    CreatedAt = (DateTime)a.CreatedDate
                }).ToList(),
        ...
        private TaskAttachmentDto MapToAttachmentDto(TaskAttachment attachment)
        {
            return new TaskAttachmentDto
            {
                ...
                CreatedAt = (DateTime)attachment.CreatedDate
            };
        }
        private async Task LogTaskAction(int taskId, string action, string description, OperationContext context)
        {
            ...
            auditLog.PerformedByUserId = context.UserId;
