...
using ValidationException = Core_API.Domain.Exceptions.ValidationException;

namespace Core_API.Application.DTOs.User.Request
{
    public class UserDto
    {
        ...
        public string PhoneNumber
        {
            ...
            set
            {
                ...
                if (!Domain.ValueObjects.PhoneNumber.TryCreate(value, out var validatedPhone, out var errorMessage))
                {
                    // Fix: Use constructor with message string
                    throw new ValidationException(errorMessage ?? "Invalid phone number format");
                }
                // Store the fully formatted phone number
                _phoneNumber = validatedPhone?.ToString() ?? value;
            }
        }
